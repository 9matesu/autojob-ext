...
try {
    ...
    if ($res.status -eq "ok") {
        Write-Host "resuMe backend already running on port 8322." -ForegroundColor Green
        exit 0
    }
} catch {}

Write-Host "Starting resuMe backend on http://127.0.0.1:8322 ..." -ForegroundColor Green
